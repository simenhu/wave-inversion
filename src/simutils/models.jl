...
"""
function string_with_coefficients_and_PML!(string_length, number_of_cells; coefficients, excitation_func, positions, pml_width=15)
    dx = string_length/(number_of_cells+1)

    pml_coeffs = dampening_coefficients(number_of_cells, pml_width)
    # c^2 = a*b 
    # c^2 = T/μ
    a = b = sqrt.(coefficients)

    function du_func(du, u, p, t)

        A_xv = CenteredDifference{1}(1, 3, dx, number_of_cells, b)
        A_xu = CenteredDifference{1}(1, 3, dx, number_of_cells, a)
        Q_v = Dirichlet0BC(Float64)
        Q_u = Dirichlet0BC(Float64)

        for i in eachindex(positions)
            u[positions[i]] = u[positions[i]] + excitation_func[i](t)
        end
...
"""
function make_initial_condition(number_of_cells)
